...
import { AdminAccountSelfUpdateFormType } from './form'
...
const importExcelFormId = 'admin-account-change-profile-image-form'

export default function AdminAccountSelfUpdateFormComponent(props: Props) {
  const navigate = useNavigate()

  const importExcelFormRef = useRef<HTMLFormElement>(null)
  const importExcelInputRef = useRef<HTMLInputElement>(null)

  const formHook = useRemixFormContext<AdminAccountSelfUpdateFormType>()
    ...
    <div className='grid grid-cols-2 gap-x-8 gap-y-2'>
      <div className='flex flex-row items-center flex-wrap gap-4 col-span-2'>
        <Form
          ...
          action={AppNav.adminAction.accountUploadProfileImage()}
        ...
        <div>
          {props.account.profileImageObjectUrl ? (
            <img
              ...
              loading='lazy'
              className='w-12 h-12 rounded-full aspect-square bg-primary border border-primary'
            />
          ) : (
            <div className='w-12 h-12 rounded-full aspect-square bg-primary flex flex-row items-center justify-center text-white'>
        ...
        <div>
          <Button
            ...
            variant='outlined'
            color='primary'
            buttonProps={{ onClick: () => navigate(AppNav.admin.accountChangePassword()) }}
          />
        </div>
      </div>
