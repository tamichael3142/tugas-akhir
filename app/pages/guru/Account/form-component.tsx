...
import { GuruAccountSelfUpdateFormType } from './form'
...
const importExcelFormId = 'guru-account-change-profile-image-form'

export default function GuruAccountSelfUpdateFormComponent(props: Props) {
  const navigate = useNavigate()

  const importExcelFormRef = useRef<HTMLFormElement>(null)
  const importExcelInputRef = useRef<HTMLInputElement>(null)

  const formHook = useRemixFormContext<GuruAccountSelfUpdateFormType>()
    ...
    <div className='grid grid-cols-2 gap-x-8 gap-y-2'>
      <div className='flex flex-row items-center flex-wrap gap-4 col-span-2'>
        <Form
          ...
          action={AppNav.guruAction.accountUploadProfileImage()}
        ...
        <div>
          <Button
            ...
            variant='outlined'
            color='primary'
            buttonProps={{ onClick: () => navigate(AppNav.guru.accountChangePassword()) }}
          />
        </div>
      </div>
