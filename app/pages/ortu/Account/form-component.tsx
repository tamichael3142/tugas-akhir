...
import { OrtuAccountSelfUpdateFormType } from './form'
...
const importExcelFormId = 'ortu-account-change-profile-image-form'

export default function OrtuAccountSelfUpdateFormComponent(props: Props) {
  const navigate = useNavigate()

  const importExcelFormRef = useRef<HTMLFormElement>(null)
  const importExcelInputRef = useRef<HTMLInputElement>(null)

  const formHook = useRemixFormContext<OrtuAccountSelfUpdateFormType>()
    ...
    <div className='grid grid-cols-2 gap-x-8 gap-y-2'>
      <div className='flex flex-row items-center flex-wrap gap-4 col-span-2'>
        <Form
          ...
          action={AppNav.ortuAction.accountUploadProfileImage()}
        ...
        <div>
          <Button
            ...
            variant='outlined'
            color='secondary'
            buttonProps={{ onClick: () => navigate(AppNav.ortu.accountChangePassword()) }}
          />
        </div>
      </div>
