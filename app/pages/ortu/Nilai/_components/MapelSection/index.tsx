...
export default function MapelSection(props: Props) {
  return (
    <div className='max-w-full overflow-x-auto p-4 bg-gray-100 rounded-2xl'>
        ...
        <thead>
          <tr>
            {props.kompetensis.map(item => (
              <th key={`th-kompetensi-${item.id}`} className='p-2 border min-w-32 h-12'>
                {item.label}
              </th>
            ))}
          </tr>
        ...
        <tbody>
          <tr>
            {props.kompetensis.map(item => {
              ...
              return (
                <td key={`td-nilai-${item.id}`} className='p-2 border min-w-32 h-12 text-center'>
