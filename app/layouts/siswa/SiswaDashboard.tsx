...
export default function SiswaDashboardLayout() {
  const loader = useLoaderData<LoaderDataSiswa>()
  const location = useLocation()
  const popup = usePopup()
  const navigate = useNavigate()
  const user = useAuthStore(state => state.user)
  const openSidebar = useSiswaPageStore(state => state.openSidebar)
  const [timeoutId, setTimeoutId] = useState<number | null>(null)

  const closeSidebar = () => useSiswaPageStore.setState({ openSidebar: false })
